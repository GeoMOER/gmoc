...
# Merge land use and crop information into shapes ------------------------------
# Federal level
vg250_lan <- readOGR(paste0(path_bkg, "vg250_0101.utm32s.shape.ebenen/vg250_ebenen/VG250_LAN.shp"),
                     "VG250_LAN")
head(vg250_lan@data)
vg250_lan@data <- merge(vg250_lan@data, lu_cp_final)

head(vg250_lan@data)
summary(vg250_lan@data)
length(unique(vg250_lan@data$DEBKG_ID))

# "Regierungsbezirk" level
vg250_rbz <- readOGR(paste0(path_bkg, "vg250_0101.utm32s.shape.ebenen/vg250_ebenen/VG250_RBZ.shp"),
                     "VG250_RBZ")
head(vg250_rbz@data)
vg250_rbz@data <- merge(vg250_rbz@data, lu_cp_final)

head(vg250_rbz@data)
summary(vg250_rbz@data)
length(unique(vg250_rbz@data$DEBKG_ID))


# "Kreis" level
vg250_krs <- readOGR(paste0(path_bkg, "vg250_0101.utm32s.shape.ebenen/vg250_ebenen/VG250_KRS.shp"),
                     "VG250_KRS")
head(vg250_krs@data)
vg250_krs@data <- merge(vg250_krs@data, lu_cp_final)

head(vg250_krs@data)
summary(vg250_krs@data)
length(unique(vg250_krs@data$DEBKG_ID))

saveRDS(vg250_krs, file = paste0(path_rdata, "vg250_krs_shp"))
